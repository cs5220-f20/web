...
![Harder: tasks have predictable structure (some DAG)](figs/lb-deps-harder.svg){width=80%}
...
- Put root in queue; **workers active = 0**
- fork
  - obtain queue lock
  - while queue has work **or workers active \> 0**
    ...
    - process $n$, mark as searched
    - obtain queue lock
    - enqueue unsearched children of $n$; **workers active --**
  - release queue lock
- join
