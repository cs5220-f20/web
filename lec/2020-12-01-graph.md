...
-   May have weights $w_V : V \rightarrow \bbR$ or
...
-   Form set $S$ with start + random nodes, $|S| = \Theta(\sqrt{n}
        \log n)$ --- long shortest paths must go through $S$ with high
    prob
...
-   Relax "light" edges (weight \< $\Delta$) which might add to current
    bucket
